...
class ObatController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        $pemeriksaan = DB::table('pemeriksaan')
            ...
            ->join('pasien', 'pemeriksaan.pasien_id', '=', 'pasien.id')
            ->where('pemeriksaan.id', $id) // Filter by specific pasien_id
            ->whereNull('pemeriksaan.deleted_at')
            ->whereNull('pasien.deleted_at')
            ->whereMonth('pemeriksaan.created_at', $currentMonth)
                ->whereYear('pemeriksaan.created_at', $currentYear)
            ->orderBy('pemeriksaan.created_at', 'asc')
            ->get();

  $obat = DB::table('obat')
            ->select('obat')
            ->where('pemeriksaan_id', $pemeriksaan[0]->id)
            ->where('pasien_id', $pemeriksaan[0]->pasien_id) // Filter by specific pasien_id
            ->whereNull('deleted_at')
            ->whereMonth('created_at', $currentMonth)
            ->whereYear('created_at', $currentYear)
            ->get();
            
        return response()->json(['pemeriksaan' => $pemeriksaan, 'obat' => $obat]);
    }
