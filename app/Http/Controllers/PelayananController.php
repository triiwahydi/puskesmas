...
class PelayananController extends Controller
{
    ...
    public function store(Request $request)
    {
       ...
       $pasien = ModelPasien::find($validatedData['pasien_id']);

        // Calculate age
        $tglLahir = Carbon::parse($pasien->tgl_lahir);
        $umur = $tglLahir->diff(Carbon::now())->format('%y Tahun %m Bulan %d Hari');
    ...
    public function show(string $id)
    {
        ...
        $pelayanan = DB::table('pelayanan')
            ->select('pelayanan.*', 'pasien.id as pasien_id', 'pasien.nik','pasien.bpjs','pasien.tgl_lahir', 'pasien.nama', 'pasien.jen_kel','pasien.umur', 'pasien.alamat') // Select all columns from both tables
            ->join('pasien', 'pelayanan.pasien_id', '=', 'pasien.id') // Join based on the foreign key relationship
            ->where('pelayanan.antrian_id', $id) // Filter by specific pasien_id
            ->whereNull('pelayanan.deleted_at')
            ->whereNull('pasien.deleted_at')
            ->whereMonth('pelayanan.created_at', $currentMonth)
            ->whereYear('pelayanan.created_at', $currentYear) // Filter for today's date in the 'pelayanan' table
